...
class CatalogController < ApplicationController # rubocop:disable Metrics/ClassLength
  ...
  configure_blacklight do |config| # rubocop:disable Metrics/BlockLength
    ...
    config.add_show_field 'item__object_type__uri', label: 'Object Type', component: AnchorMetadataComponent
    config.add_show_field 'item__identifier__ids', label: 'Identifier', component: ListMetadataComponent
    config.add_show_field 'item__rights__uri', label: 'Rights Statement', component: AnchorMetadataComponent
  ...
  def show
    ...
    publishable = @document.has_key?(:item_rdf_type__uris)
    @published = publishable == true ? document[:item_rdf_type__uris].include?('http://vocab.lib.umd.edu/access#Published') : false
  end
  ...
  private
    ...
    def mirador_displayable?(document)
      return false if !document.has_key?(:rdf_type__facet)

      rdf_types = document[:rdf_type__facet]
      component = document[:content_model_name__str]

      rdf_types.include?('pcdm:Object') && (component != 'Article')
    end
